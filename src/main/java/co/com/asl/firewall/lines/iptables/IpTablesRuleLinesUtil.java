...
public final class IpTablesRuleLinesUtil {
  ...
  public static Stream<String> execute(CIDRAddressV4 cidrAddressV4,
    ...
    switch (operation) {
      ...
      case DROP:
        return Stream.of(
            String.format("-A user-output -d %s -j %s", cidrAddressV4,
                operation.name().toUpperCase())
        );
      case REJECT:
        return Stream.of(
            String.format("-A user-output -d %s -j %s --reject-with icmp-port-unreachable",
                cidrAddressV4,
                operation.name().toUpperCase())
        );
    }

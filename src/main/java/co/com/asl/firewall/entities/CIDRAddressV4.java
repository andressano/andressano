...
public class CIDRAddressV4 implements Comparable<CIDRAddressV4> {
    ...
    private void set(String ip) {
        ...
        for (int i = 0; i < NUMBER_OF_GROUPS; i++) {
            ...
            if (i < NUMBER_OF_GROUPS - 1) this.ip <<= BYTES_PER_GROUP;
        }

        if (parts.length == 2) setMask(Short.parseShort(parts[1]));
        else setMask(TOTAL_BYTES);
    }
    ...
    public int compareTo(CIDRAddressV4 cidrAddressV4) {
        ...
        for (int i = NUMBER_OF_GROUPS - 1; i >= 0 && compare == 0; i--) {
            compare = Integer.compare(getIp() >>> (BYTES_PER_GROUP * i), cidrAddressV4.getIp() >>> (BYTES_PER_GROUP * i));
        }
        if (compare == 0) return Integer.compare(cidrAddressV4.getMask(), getMask());
        return compare;
    }

    public Optional<CIDRAddressV4> combineIfPossible(CIDRAddressV4 cidrAddressV4) {
        if (this.getMask() > 0 && cidrAddressV4.getMask() > 0 && this.getMask() < 32 && cidrAddressV4.getMask() < 32 && this.getMask() != cidrAddressV4.getMask())
        ...
        if (minIp == andIp && difIp == 1)
            return Optional.of(new CIDRAddressV4(this.getIp() & cidrAddressV4.getIp(), (short) (this.getMask() - 1)));
        return Optional.empty();
    }

    public boolean contains(CIDRAddressV4 cidrAddressV4) {
        return this.getMask() <= cidrAddressV4.getMask() && (cidrAddressV4.getIp() & MAX_VALUE << (TOTAL_BYTES - this.getMask())) == this.getIp();
    }
    ...
    public String toString() {
        final StringBuilder binaryString = new StringBuilder(StringUtils.leftPad(Integer.toBinaryString(this.getIp()), TOTAL_BYTES, '0'));
